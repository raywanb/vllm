...
"""Inference-only LLaMA model with SwiftKV support for KV cache sharing."""
...
class LlamaSwiftKVAttention(nn.Module):
    """LlamaAttention with SwiftKV support for KV cache sharing."""
    ...
    def _init_rotary_emb(self, config: LlamaSwiftKVConfig,
...
class LlamaSwiftKVDecoderLayer(nn.Module):
    """LlamaDecoderLayer with SwiftKV support for KV cache sharing."""
...
class LlamaSwiftKVModel(nn.Module):
    """LlamaModel with SwiftKV support for KV cache sharing."""
    ...
    def __init__(self,
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: LlamaSwiftKVDecoderLayer(
                config=config,
                cache_config=cache_config,
                quant_config=quant_config,
                prefix=prefix,
    ...
    def load_weights(self, weights: Iterable[tuple[str,
        ...
        """Load weights with SwiftKV weight mapping for q_proj_swiftkv."""
        # Create a mapping for SwiftKV weights
        swiftkv_weight_mapping = {}
        
        for name, weight in weights:
            # Check if this is a SwiftKV weight (q_proj_swiftkv)
            if any(swiftkv_suffix in name for swiftkv_suffix in ['q_proj_swiftkv', 'gate_proj_swiftkv', 'up_proj_swiftkv', 'down_proj_swiftkv', 'q_norm_swiftkv', 'k_norm_swiftkv', 'norm_swiftkv', 'input_layernorm_swiftkv', 'post_attention_layernorm_swiftkv']):
                # Extract layer index from the weight name
                # Expected format: model.layers.{layer_idx}.self_attn.q_proj_swiftkv.weight
                parts = name.split('.')
                layer_idx = None
        ...
        # Use the standard LlamaModel weight loading logic with mapped weights
        ...
        for name, loaded_weight in swiftkv_weight_mapping.items():
...
class LlamaSwiftKVForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsEagle3):
    """LlamaForCausalLM with SwiftKV support for KV cache sharing."""
    ...
    def __init__(self,
        ...
        self.model = LlamaSwiftKVModel(vllm_config=vllm_config,
