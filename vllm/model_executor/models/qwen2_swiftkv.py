...
# from .qwen2 import Qwen2MLP as Qwen2MLP
# from .qwen2 import Qwen2DecoderLayer  # reuse as head-layer implementation
...
# class Qwen2SwiftKVAttention(nn.Module):
#     """
#     Tail-layer attention for Qwen2 that:
#       - Projects Q only (q_proj_swiftkv).
#       - Uses precomputed K/V injected by the model (set each step).
#       - Applies q/k per-head RMSNorm and RoPE (same as Qwen2Attention).
...
#         self.self_attn = Qwen2SwiftKVAttention(
...
#         "inputs_embeds": 0,
#     })
# class Qwen2SwiftKVModel(nn.Module):

#     def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
#         super().__init__()
#         config: Qwen2Config = vllm_config.model_config.hf_config
...
#             ) if idx < config.num_key_value_layers else Qwen2SwiftKVDecoderLayer(
#                 config=config,
#                 quant_config=quantization_or_none(quant_config),
#                 prefix=f"{prefix}.layers.{idx}",
#             )
...
#             layer: Qwen2SwiftKVDecoderLayer = self.layers[idx]
#             attn: Qwen2SwiftKVAttention = layer.self_attn
...
#         print("self.config.num_key_value_layers", self.config.num_key_value_layers)
...
#             stacked_params_mapping.extend([ (f"{prefix}.qkv_proj", f"{prefix}.q_proj", "q"), (f"{prefix}.qkv_proj", f"{prefix}.k_proj", "k"), (f"{prefix}.qkv_proj", f"{prefix}.v_proj", "v"), ])

#         for layer_idx in range(self.config.num_key_value_layers, self.config.num_hidden_layers): 
#             prefix = f".{layer_idx}.self_attn" 
#             stacked_params_mapping.extend([ (f"{prefix}.kv_proj_swiftkv", f"{prefix}.k_proj_swiftkv", "k"), (f"{prefix}.kv_proj_swiftkv", f"{prefix}.v_proj_swiftkv", "v"), ])
...
# class Qwen2SwiftKVForCausalLM(nn.Module, SupportsLoRA, SupportsPP, SupportsEagle3):
